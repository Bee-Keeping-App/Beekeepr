...
import { FailedValidationError } from '../classes/errors.class.js';
...
export default (schema: AnyZodObject | ZodEffects<any>) => { // schema is the joi validator passed in the route file
    return (req: Request, res: Response, next: NextFunction) => {
        try {   // note the try/catch statement, as middlewares are outside of the controller scope 
            ...
            if (error instanceof z.ZodError)
                // You can pass the Zod error details to your custom class
                return next(new FailedValidationError(error.message));
