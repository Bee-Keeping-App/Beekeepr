import { jest, describe, xtest, expect, beforeEach } from '@jest/globals';
...
jest.unstable_mockModule('../../backend/services/weather.service.js', () => ({
...
describe('GET /api/weather/current', () => {
    ...
    xtest('returns 200 with weather data for valid zip', async () => {
    ...
    xtest('returns 400 for missing zip', async () => {
        const res = await request(app)
            .get('/api/weather/current');
    ...
    xtest('returns 400 for malformed zip (non-numeric)', async () => {
        const res = await request(app)
            .get('/api/weather/current')
            .query({ zip: 'abcde' });
    ...
    xtest('returns 400 for zip with wrong length', async () => {
        const res = await request(app)
            .get('/api/weather/current')
            .query({ zip: '1234' });
    ...
    xtest('returns 500 when service throws', async () => {
...
describe('GET /api/weather/historical', () => {
    ...
    xtest('returns 200 with historical data for valid params', async () => {
    ...
    xtest('returns 400 for missing start_date', async () => {
        const res = await request(app)
            .get('/api/weather/historical')
    ...
    xtest('returns 400 for missing end_date', async () => {
        const res = await request(app)
            .get('/api/weather/historical')
    ...
    xtest('returns 400 for malformed date format', async () => {
        const res = await request(app)
            .get('/api/weather/historical')
